...
<div class="grid grid_layout" id="appWallet">
            ...
            <div class="cabinet">
                <div class="header header_margin-1 content__header">
                    <a href="#" class="header__nav-button header__nav-button_back">
                        <svg class="icon header__nav-button-icon"><use xlink:href="/assets/images/icons/icons.svg#icon-arrow-2"></use></svg>
                    </a>
                    <div class="headline">
                    ...
                    <div class="header__text">
                        <p>
                            ...
                            Из «Моего кошелька» периодически (раз в месяц) происходит списание баллов и их перевод в «премию за квалификацию».
                            Баллы рейтинговые – начисляются за активность на портале (за происхождение информационных и обязательных обучений).
                        </p>
                    </div>
                </div>
                <div class="wallet cabinet__wallet js-wallet">
                            ...
                            <div class="wallet__tabs-content-item active">
                                <div class="transactions wallet__transactions" data-autoload="true">
                                    <div class="transactions__list" v-for="item in allList">
                                        <div class="transaction-mini transactions__item" :class="{'transaction-mini_minus': item.direction == 'down', 'transaction-mini_plus': item.direction == 'up'}">
                                            <div class="transaction-mini__grid transaction-mini__grid_1">
                                                <div class="transaction-mini__grid-item transaction-mini__grid-item_1">
                                                    ...
                                                    <a :href="item.url" class="transaction-mini__title" v-html="item.name" v-if="item.url.length > 1">
                                                    </a>
                                                    <div class="transaction-mini__number">
                                                        <div class="transaction-mini__number-icon">
                                                            <svg class="icon transaction-mini__number-icon-canvas"><use xlink:href="/assets/images/icons/icons.svg#icon-arrow-2"></use></svg>
                                                        </div>
    ...
    const appUserReg = BX.Vue3.BitrixVue.createApp({
        ...
        methods: {
            init(){
                ...
                this.el.querySelectorAll('.wallet__tabs').each(function() {
                    let tabs = {
                        $el: this,
                    };

                    _this.tabs.nav = {
                        $items: this.el.querySelectorAll('.wallet__tabs-nav-item'),
                    };

                    _this.tabs.content = {
                        $items: this.el.querySelectorAll('.wallet__tabs-content-item'),
                    };

                    _this.tabs.nav.$items.on('click', function(event) {
                        let item = {
                            $el: this,
                        };
                        ...
                        _this.content.$items.eq(item.index).addClass('active').siblings().removeClass('active');
                    });
                });
            },
            getInfo(){
                const _this = this
                this.send('getInfo', {page: 1, count: 1})
